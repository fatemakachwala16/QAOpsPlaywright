...
test('Test 1 — Banner IS visible when 6 events are returned', async ({ page }) => {
    ...
    await page.route('https://api.eventhub.rahulshettyacademy.com/api/events**',
        async route => {
            ...
            let body: any = JSON.stringify(SIX_EVENTS_RESPONSE)
            route.fulfill({
                response,
                body,
                status: 200,
    ...
    expect(page.locator('[data-testid="event-card"]').first()).toBeVisible()
    await expect(page.locator('[data-testid="event-card"]')).toHaveCount(6)

    const banner = page.getByText(
        /sandbox holds up to/i
    );
...
test('Test 2 — Banner is NOT visible when 4 events are returned', async ({ page }) => {
    ...
    await page.route('https://api.eventhub.rahulshettyacademy.com/api/events**',
        async route => {
            const response = await page.request.fetch(route.request())
            let body: any = JSON.stringify(FOUR_EVENTS_RESPONSE)
            route.fulfill({
                response,
                body,
                status: 200,
